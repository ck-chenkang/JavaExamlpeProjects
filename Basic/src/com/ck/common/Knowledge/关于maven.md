参考链接：[Maven基础入门与核心知识 ](https://www.cnblogs.com/bndong/p/9762067.html)

## 什么是maven

maven可以通过一小段描述信息（配置）来管理项目构建，帮助我们管理、构建项目

有一个原则：约定大于配置

## 下载安装及配置

百度搜索maven，http://maven.apache.org/download.cgi
binary zip archive和link 下面的 apache-maven-3.8.1-bin.zip
...
G:groupId 一般来代表项目名称，如：org.springframework

A:artifactId 一般来代表模块名称，如：spring-webmvc

V:version 模块版本，如：4.3.7.RELEASE

Maven使用scope来指定当钱包的依赖范围和依赖的传递性，

## IDEA配置

参考链接：[IDEA配置Maven（详细版）](https://blog.csdn.net/qq_42057154/article/details/106114515)  CSDN超高收藏

在project setting里面搜索maven，有三个
...
setting.xml主要用于maven的运行环境等一系列的通用属性，是全局级别的配置文件

pom.xml主要描述了项目级别的maven坐标、依赖关系等，是项目级别的配置文件。
